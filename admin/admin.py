...
def login():
    ...
    if request.method == "POST":
        if request.form['name'] == 'admin' and request.form['psw'] == '12345':
            login_admin()
            return redirect(url_for('.index'))
        else:
...
def listpubs():
    ...
    return render_template('admin/listpubs.html', menu=menu, list=list, title='Публикации')
...
def listusers():
    ...
    if db:
        try:
            ...
            print("Ошибка получения данных из БД" + str(e))
    return render_template('admin/listusers.html', menu=menu, list=list, title='Пользователи')